  ...
  function render(o) {
    ...
    const id = VQ.mount(el => {
      ...
      if (o.towns !== false) (D.places || []).forEach(t => { if (t.km < ka - 30 || t.km > kb + 30) return; if (full && t.minor && W < 900) return; const p = pos(t.km), big = t.major; const below = t.below ? 1 : -1; const edge = !full && (p[0] < x0 + 44 * s || p[0] > x0 + vw - 44 * s);
        ...
        if (big && o.ends !== false && (t.km === 0 || t.km === KM)) labels += `<text x="${p[0] + (t.km === 0 ? -6 : 8) * s}" y="${p[1] + (t.km === 0 ? -22 : 36) * s}" text-anchor="${t.km === 0 ? 'start' : 'end'}" style="font-size:${f(15)}px;font-weight:800;fill:#0b2559;paint-order:stroke;stroke:#fff;stroke-width:${f(4)}px">${t.km === 0 ? '← ' + t.name : t.name + ' →'}</text>`;
