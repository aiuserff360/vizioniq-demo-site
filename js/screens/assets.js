...
(() => {
  ...
  const gantt = () => `<div class="legend" style="margin-bottom:8px"><span><i class="sq" style="background:var(--s1)"></i>Preventive</span><span><i class="sq" style="background:var(--s2)"></i>Corrective</span><span><i class="sq" style="background:var(--crit)"></i>Overdue work, now booked</span></div>
    <div style="display:grid;grid-template-columns:150px minmax(0,1fr);font-size:12px;border:1px solid var(--line-2);border-radius:8px;overflow:hidden">
      ...
      <div style="display:grid;grid-template-columns:repeat(7,minmax(0,1fr));gap:0;border-top:1px solid var(--line-2);align-items:center;background:linear-gradient(90deg,var(--blue-50) 0,var(--blue-50) 14.2857%,transparent 14.2857%)">${plan.filter(p => p[0] === ci).map(p => `<div data-tip="<b>${p[5]}</b><br>${c[0]} ${c[1]} · ${days[p[1]][0]} ${days[p[1]][1]}${p[2] > 1 ? ' – ' + days[p[1] + p[2] - 1][0] + ' ' + days[p[1] + p[2] - 1][1] : ''} Sep" style="grid-row:1;grid-column:${p[1] + 1} / span ${p[2]};margin:5px 3px;padding:4px 8px;border-radius:5px;background:${kindCol[p[4]]};color:#fff;font-size:11px;font-weight:600;line-height:1.2;white-space:normal;overflow-wrap:anywhere">${p[3]}</div>`).join('')}</div>`).join('')}
    </div>`;
